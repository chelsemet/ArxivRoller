...
def load_stop_words(stop_word_file):
    ...
    if stop_word_file == None:
        return list(nltk.corpus.stopwords.words('english'))
    stop_words = []
...
class Rake(object):
    ...
    def get_keywords_of_topic_abstracts(self, list_keywords):

        texts_with_keywords = []
        keyword_strings = []

        for topic_abstract_text in list_keywords:
            keywords_list = []
            keywords_list = self.run(topic_abstract_text)
            keywords_total = len(keywords_list)
            keywords_ranks = []
            string = ''

            chosen = int(keywords_total / 3)
            
            for keyword in keywords_list[0:chosen]:
                keywords_ranks.append(keyword[0])

            for word in range((len(keywords_ranks)-1)):
                string += keywords_ranks[word]
                string += ','
            string = string + keywords_ranks[0]
            
            texts_with_keywords.append((topic_abstract_text, keywords_ranks))
            keyword_strings.append((topic_abstract_text, string))

        # result_data = pd.DataFrame(np.array(keyword_strings, dtype = object), columns=['topic_and_abstract', 'keywords'])
        # result_data.to_csv("keywords.csv")
