...
def query_page(search_query=None, id_list=None, start=0, max_results=100, session=None, sortOrder="descending"):
    """
    Download a single page of results from arXiv's API and returns an iterator
    of parsed results.
    """
    url_args = {"start": start, "max_results": max_results, "sortBy": "lastUpdatedDate", "sortOrder": sortOrder}
    ...
    if session is None:
        ...
        response = session.get(ROOT_URL + "query?" + urlencode(url_args), headers=headers)
    # print("Call Arxiv API: "+ROOT_URL + "query?" + urlencode(url_args))
